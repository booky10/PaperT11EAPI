...
public class PropertyMapWrapper extends Wrapper {
	...
	static final Class<?> Multimap           = CLASS_RESOLVER.resolveSilent("net.minecraft.util.com.google.common.collect.Multimap", "com.google.common.collect.Multimap");
	static final Class<?> ForwardingMutlimap = CLASS_RESOLVER.resolveSilent("net.minecraft.util.com.google.common.collect.ForwardingMultimap", "com.google.common.collect.ForwardingMultimap");

	static final MethodResolver MultimapMethodResolver           = new MethodResolver(Multimap);
	static final MethodResolver ForwardingMultimapMethodResolver = new MethodResolver(ForwardingMutlimap);
	...
	public PropertyMapWrapper(JsonArray jsonArray) {
		...
		for (JsonElement next : jsonArray) {
			if (next instanceof JsonObject) {
				...
				put(jsonObject.get("name").getAsString(),
                        new PropertyWrapper(jsonObject.get("name").getAsString(),
                                jsonObject.get("value").getAsString(), jsonObject.has("signature") ?
                                jsonObject.get("signature").getAsString() : null));
			}
		}
	}
	...
	public PropertyMapWrapper(JSONArray jsonArray) {
		...
		for (Object next : jsonArray) {
			if (next instanceof JSONObject) {
				...
				put((String) jsonObject.get("name"), new PropertyWrapper((String) jsonObject.get("name"),
                        (String) jsonObject.get("value"), jsonObject.containsKey("signature") ? ((String) jsonObject.get("signature")) : null));
			}
		}
	}
