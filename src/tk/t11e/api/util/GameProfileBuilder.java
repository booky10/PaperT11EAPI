...
public class GameProfileBuilder {
    ...
    private static final HashMap<UUID,Property> propertyCache = new HashMap<>();

    private static final Gson gson = new GsonBuilder().disableHtmlEscaping().registerTypeAdapter(UUID.class,
            new UUIDTypeAdapter()).registerTypeAdapter(GameProfile.class, new GameProfileSerializer()).registerTypeAdapter(PropertyMap.class, new PropertyMap.Serializer()).create();
    ...
    public static Property getTexture(UUID uuid) throws IOException {
        if(propertyCache.containsKey(uuid))
            return propertyCache.get(uuid);
        else {
            InputStreamReader reader = new InputStreamReader(new URL(String.format(SERVICE_URL,
                    UUIDTypeAdapter.fromUUID(uuid))).openStream());
            JsonObject textureProperty = new JsonParser().parse(reader).getAsJsonObject().get("properties")
            ...
            propertyCache.put(uuid,property);
            return property;
        }
    }
    ...
    public static GameProfile fetch(UUID uuid, boolean forceNew) throws IOException {
        if (!forceNew && cache.containsKey(uuid) && cache.get(uuid).isValid()) {
            ...
            HttpURLConnection connection = (HttpURLConnection) new URL(String.format(SERVICE_URL,
            ...
            if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                JsonObject error = (JsonObject) new JsonParser().parse(new BufferedReader(new
                        InputStreamReader(connection.getErrorStream())).readLine());
                throw new IOException(error.get("error").getAsString() + ": " + error
    ...
    public static GameProfile getProfile(UUID uuid, String name, String skinUrl, String capeUrl) {
        ...
        profile.getProperties().put("textures", new Property("textures", Base64Coder
                .encodeString(String.format(cape ? JSON_CAPE : JSON_SKIN, args.toArray(new Object[0])))));
        return profile;
    }
    ...
    private static class GameProfileSerializer implements JsonSerializer<GameProfile>,
        ...
        public GameProfile deserialize(JsonElement json, Type type, JsonDeserializationContext context)
            ...
            UUID id = object.has("id") ? (UUID) context.deserialize(object.get("id"),
                    UUID.class) : null;
            String name = object.has("name") ? object
                    .getAsJsonPrimitive("name").getAsString() : null;
            GameProfile profile = new GameProfile(id, name);

            if (object.has("properties")) {
                for (Entry<String, Property> prop : ((PropertyMap) context.deserialize(object
                        .get("properties"), PropertyMap.class)).entries()) {
